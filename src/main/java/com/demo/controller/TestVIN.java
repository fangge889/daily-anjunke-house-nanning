...
public class TestVIN {
	...
	public static void main(String[] args) {
		boolean result = checkVIN("LSVCE6A41BN211634");
		...
		if (vin == null || uppervin.indexOf("O") >= 0 || uppervin.indexOf("I") >= 0) {
			...
			if (vin.length() == 17) {
				...
				for (int i = 0; i < vinArr.length; i++) {
					// VIN码从从第一位开始，码数字的对应值×该位的加权值，计算全部17位的乘积值相加</span>
					amount += vinMapValue.get(vinArr[i]) * vinMapWeighting.get(i + 1);
				}
				// 乘积值相加除以11、若余数为10，即为字母Ｘ</span>
				if (amount % 11 == 10) {
					if (vinArr[8] == 'X') {
						reultFlag = true;
					} else {
					...
					// 计算全部17位的乘积值相加除以11，所得的余数，即为第九位校验值</span>
